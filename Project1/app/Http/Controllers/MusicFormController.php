...
class MusicFormController extends Controller
{
    ...
    public function musicAdd($id){
        ...
        $playlist = Playlist::select('name','id')->where('user_id','=',Auth::id())->whereHas('musiclist', function ($query) use ($id) {
            $query->where('music_id','=',$id);
        })->get();
        $allList = Playlist::select('name','id')->where('user_id','=',Auth::id())->get();
        // dd($allList,$playlist);
        if(count($playlist) == 0){
            $playlist = $allList;
